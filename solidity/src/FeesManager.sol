...
contract FeesManager is IFeesManager, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint16 => mapping(address => uint256)) public depositedAmountByEpoch;
    mapping(uint16 => mapping(address => mapping(address => uint256))) public claimedAmountByEpoch;
    ...
    function initialize(uint16 firstEpoch, address[] calldata nodes, uint256[] calldata amounts) public initializer {
    ...
    function claimFeeByEpoch(address token, uint16 epoch) external {
        ...
        if (claimedAmountByEpoch[epoch][token][sender] > 0) revert AlreadyClaimed();
        uint256 amount = (depositedAmountByEpoch[epoch][token] * stakedAmountByEpoch[epoch][sender]) /
    ...
    function registerAndAdvanceEpoch(address[] calldata nodes, uint256[] calldata amounts) external onlyOwner{
        currentEpoch += 1;
        for (uint i = 0; i < nodes.length; i++) {
            stakedAmountByEpoch[currentEpoch][nodes[i]] = amounts[i];
            totalStakedAmountByEpoch[currentEpoch] += amounts[i];
    ...
    /// @inheritdoc IFeesManager
    function calculateFee(address token, uint256 amount, bytes4 destinationChainId) public view returns (uint256) {
        Fee memory info = feeInfoByAsset[token];
        if (info.minFee == 0 && info.originChainId == 0) revert UnsupportedToken();
        uint256 fee = (amount * (destinationChainId == feeInfoByAsset[token].originChainId ? 2500 : 1000)) / 1000000;
        return fee < feeInfoByAsset[token].minFee ? feeInfoByAsset[token].minFee : fee;
    }
    ...
    /// @inheritdoc IFeesManager
    function depositFeeForEpoch(address token, uint256 amount, uint16 epoch) public {
        if (epoch < currentEpoch) revert InvalidEpoch();
        depositedAmountByEpoch[epoch][token] += amount;
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
    }

    function setFee(address token, uint256 minAmount, bytes4 originChainId) external onlyOwner {
