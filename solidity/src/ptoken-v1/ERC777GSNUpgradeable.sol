    ...
    ERC777Upgradeable
{
    ...
    function _msgSender()
        ...
        view
        virtual
        override(ContextUpgradeable, GSNRecipientUpgradeable)
    ...
    function acceptRelayedCall(
        ...
        if (
            ...
            return
                _approveRelayedCall(
                    abi.encode(feeRate, from, transactionFee, gasPrice)
                );
        } else {
    ...
    function _postRelayedCall(
        ...
        uint256 overestimation = _computeCharge(
            _POST_RELAYED_CALL_MAX_GAS.sub(gsnExtraGas),
            gasPrice,
            transactionFee
        );
        uint fee = actualCharge.sub(overestimation).mul(feeRate).div(
            GSN_RATE_UNIT
        );
