...
library RLP {
  function encode(
        ...
        bytes memory destinationChainIdBytes = encodeBytes32(destinationChainId); // length 33
        ...
        for (uint8 i = 0; i < 33; i++) {
            ...
            rlpEncodedBytes[offset + 33 + i] = bytes1(destinationChainIdBytes[i]);
            rlpEncodedBytes[offset + 66 + i] = bytes1(amountBytes[i]);
        }
    }

    function encodeBytes32(bytes32 b) internal pure returns (bytes memory encodedBytes32) {
    ...
    function encodeAddress(address a) internal pure returns (bytes memory encodedAddress) {
    ...
    function encodeData(bytes memory data) internal pure returns (bytes memory encodedData) {
